...
function displayCampaignMenu() {
    logger.section('SELECT CAMPAIGN');
...
async function runAllAccounts(tokens, proxies) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        logger.section(`Account ${i + 1}/${totalAccounts}`);
    ...
    logger.summary(`Run complete for this campaign. ${successfulAccounts}/${totalAccounts} accounts successful. Total submissions: ${totalSubmissions}`);
}
...
async function showMainMenu() {
    ...
    return new Promise((resolve) => {
        const askQuestion = () => {
            ...
            console.log(`${colors.yellow}0. Exit${colors.reset}\n`);
            rl.question(`${colors.cyan}Enter your choice: ${colors.reset}`, (answer) => {
                const choice = parseInt(answer.trim());
...
async function startBot() {
    ...
    // Get user configuration once at the start
    const choice = await showMainMenu();
    
    let runFunction;
    
    if (choice === 1) {
        runFunction = runAllCampaigns;
        logger.info("Mode selected: Run all campaigns.");
    } else if (choice === 2) {
        await selectCampaign();
        runFunction = runSingleCampaign;
        logger.info(`Mode selected: Run single campaign (${SELECTED_CAMPAIGN.campaign_name}).`);
    } else {
        logger.info('Exiting...');
        process.exit(0);
    }
    ...
    while (true) {
        ...
        logger.section('CYCLE COMPLETE - WAITING FOR NEXT RUN');
        logger.info(`Next run will start automatically at midnight.`);
        await countdown(nextReset.getTime(), 'Next run countdown');
...
if (require.main === module) {
    startBot().catch(error => {
        logger.critical(`Bot execution failed: ${error.message}`);
        process.exit(1);
    });
}
